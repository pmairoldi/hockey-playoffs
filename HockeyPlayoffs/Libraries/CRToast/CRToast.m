...
-(BOOL)isEqual:(id)object {
    ...
    CRToast *notification = (CRToast *)object;

    if ([self.options isEqualToDictionary:notification.options]) {
        return YES;
    }
...
@property BOOL preventNotifications;
@end
...
+(void)preventNotifications:(BOOL)preventNotifications {
    
    CRToastManager *manager = [self manager];
    manager.preventNotifications = preventNotifications;
    
    if (manager.preventNotifications) {

        if (manager.showingNotification) {
            [manager dismissNotification:NO];
        }
        
        if ([manager.notifications count] > 0) {
            [manager.notifications removeAllObjects];
        }
    }
}
...
- (void)addNotification:(CRToast*)notification {
    ...
    BOOL preventNotifications = self.preventNotifications;
